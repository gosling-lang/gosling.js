...
export default class VcfParser {
    ...
    getParser() {
        ...
        const parseLine = (line: string, chromStart: number, prevPos?: number) => {
            ...
            const vcfTile = this.#recordToTile(vcfRecord, chromStart, prevPos);
            return vcfTile;
        };
        return parseLine;
    }

    #getMutationType(ref: string, alt?: string) {
    ...
    #getSubstitutionType(ref: string, alt?: string) {
        switch (ref + alt) {
            case 'CA':
            case 'GT':
                return 'C>A';
     ...
     */
    #recordToTile(vcfRecord: VcfRecord, chrPos: number, prevPos?: number) {
        ...
        const MUTTYPE = this.#getMutationType(vcfRecord.REF, ALT);
        const SUBTYPE = this.#getSubstitutionType(vcfRecord.REF, ALT);
