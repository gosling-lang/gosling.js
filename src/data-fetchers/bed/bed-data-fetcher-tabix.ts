...
class BedDataFetcher implements TabularDataFetcher<BedTile> {
    ...
    constructor(HGC: import('@higlass/types').HGC, config: BedData & { assembly: Assembly }) {
    ...
    fetchTilesDebounced(receivedTiles: (tiles: Record<string, BedTile>) => void, tileIds: string[]) {
    ...
    async sendFetch(receivedTiles: (tiles: Record<string, BedTile>) => void, tileIds: string[]) {
