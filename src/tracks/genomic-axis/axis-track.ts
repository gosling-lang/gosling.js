...
function createTickText(text: string, style: Partial<PIXI.ITextStyle>): TickText {
    return Object.assign(new PIXI.Text(text, style), {
...
export class AxisTrackClass extends PixiTrack<AxisTrackOptions> {
    ...
    hideMousePosition?: ReturnType<typeof showMousePosition>;
    ...
    constructor(context: AxisTrackContext, options: AxisTrackOptions) {
        ...
        const chromLengths: { [k: string]: number } = {
            ...computeChromSizes(assembly).size
        };
        ...
        Object.keys(computeChromSizes(assembly).interval).forEach(k => {
            cumPositions.push({
                chr: k,
                pos: computeChromSizes(assembly).interval[k][0]
            });
        });
    ...
    addCurvedText(textObj: PIXI.Text, cx: number) {
        ...
        const txtStyle = new PIXI.TextStyle(this.pixiTextConfig);
        const metric = PIXI.TextMetrics.measureText(textObj.text, txtStyle);
        ...
        textObj.updateText();
        const rope = new PIXI.SimpleRope(textObj.texture, ropePoints);
        return rope;
    }
