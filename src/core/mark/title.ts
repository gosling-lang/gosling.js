...
export function drawLinearTitle(
    ...
    const styleConfig = getTextStyle({
        ...
        size: theme.track.titleFontSize, // seems to use much larger fonts
    ...
    /* Background */
    g.beginFill(colorToHex('white'), 0.4);
    g.lineStyle(1, colorToHex('red'), 0, 0);
    g.drawRect(0, 0, metric.width, metric.height);
...
export function drawCircularTitle(
    ...
    if (spec.layout !== 'circular') {
        // Wrong function call, this is for circular tracks.
        drawLinearTitle(HGC, trackInfo, tile, model, theme);
        return;
    }
